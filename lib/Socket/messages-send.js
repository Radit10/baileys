...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            const removeExternalAdReply = obj => {
    if (!obj || typeof obj !== 'object') return

    if (obj.contextInfo?.externalAdReply) {
        delete obj.contextInfo.externalAdReply

        if (!Object.keys(obj.contextInfo).length) {
            delete obj.contextInfo
        }
    }

    for (const key in obj) {
        if (typeof obj[key] === 'object') {
            removeExternalAdReply(obj[key])
        }
    }
}
removeExternalAdReply(content)
